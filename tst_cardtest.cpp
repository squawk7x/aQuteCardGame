...
TEST_F(
  ...
  // The value should default to 0 since the rank "2" is not defined in setValue
  EXPECT_EQ(invalidCard.value(), 0);
}

// Test value for all suits (since the value does not depend on suit, only rank)
TEST_F(
